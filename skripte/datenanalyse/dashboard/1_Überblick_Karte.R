...
sql_bundesländer <- glue_sql("
  SELECT land, bundesland, stadt, MIN(datum_scraping) as min_scraping
  FROM analysedaten
...
Städte_aufb <- Städte %>%
  ...
  mutate(min_scraping = as.Date(min_scraping),
         ...
         tooltip = glue("<b style='font-weight:700; font-size:16px; margin-bottom:1.5px; display:block;'>{stadt}</b>Bundesland: {bundesland}<br>Erhebungsstart: {min_scraping}"))
...
Karte_erhobeneStädte <- ggplot() +
  ...
  geom_sf_interactive(data = Städte_aufb, 
                      ...
                      alpha = 0.5) +
  geom_sf_interactive(data = Stadt, 
                      aes(data_id = data_id, tooltip = tooltip),
                      size = 16, color = "black", stroke = 2,
...
my_plot_ggplot <- image_read(glue("C:\\Users\\hellm\\Desktop\\WG-Gesucht_Analyse\\Daten\\Wappen\\{params$stadt}_wappen.png")) %>% 
  image_ggplot(interpolate = TRUE) +
  labs(title = glue("Stadtwappen von {params$stadt}")) +
  theme(plot.title = element_textbox_simple(halign = 0.5, size = 20, 
                                            margin = margin(b=15)))

plot_karte <- Karte_erhobeneStädte + 
  inset_element(my_plot_ggplot, 
                left = 0.65, right = 1, 
