...
Abb_Dichte <- ggplot(dichte_df, aes(x = x, y = y)) +
  ...
  annotate(
    "text",
    x = median(WGdaten_trim$gesamtmiete)+42.5 , y = 0.005, 
    label = glue("Median: {median(WGdaten_trim$gesamtmiete)}€"), 
    hjust = 0, color = "gray35") +
  geom_curve(x = median(WGdaten_trim$gesamtmiete)+40, 
             xend = median(WGdaten_trim$gesamtmiete)+5,  
             y = 0.005, yend = 0.00475,
             arrow = arrow(length = unit(0.1, "cm"), type = "closed"),
...
file_save <- "Abbildungen/1_Präsentation_Linke_Kassel/Ridgeline_Kassel_3.png"
