...
sql <- glue_sql("
  SELECT bundesland, stadt, gesamtmiete, datum_scraping
  FROM analysedaten
  WHERE (befristungsdauer IS NULL OR befristungsdauer >= 60)
    AND gesamtmiete IS NOT NULL
...
dichte_deutschland <- dichte_df %>%
  ...
  theme(legend.position = "none", 
        ...
        axis.title.x = element_text(margin = margin(t = 10), family = axis_title_family,
                                    color = axis_title_color, size=14,  face = "bold"),
        axis.text.x = element_text( family = axis_text_family, color = axis_text_color,
                                    margin = margin(t=7.5), size = 10),
        axis.text.y = element_markdown(family = axis_text_family, 
                                       size = c(rep(5, 9),0, 12, 0,rep(5, 41)),
                                       color = c(rep("gray30", 10), "gray" ,rep("gray30", 44))))
...
file_save <- "Abbildungen/1_Präsentation_Linke_Kassel/Ridgeline_Stadtteile_2.png"
