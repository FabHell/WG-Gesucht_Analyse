...
Plot_geschlecht <- Daten_Staedte_geschlecht %>%  
  ...
  geom_col(fill = panel_background_color, 
           color = panel_background_color, linewidth = 0.25) +
  geom_text(
    data = Daten_Staedte_geschlecht_text,
    aes(y=geschlecht_ges, x=x_pos, label = label_1), inherit.aes = F,
    hjust = 0, family = "domine", color = panel_background_color) +
  geom_text(
    data = Daten_Staedte_geschlecht_text,
  ...
  theme(legend.position = "none",
        ...
        axis.line = element_line(color = panel_background_color),
...
Plot_waffle_w <- Daten_waffle_w %>%
  ...
  geom_waffle(n_rows = 4, size = 0.5, colour =  panel_background_color, 
              flip = T, linewidth = 2) +
  scale_fill_manual(
...
file_save <- "/home/fabian/Schreibtisch/WG-Gesucht_Analyse/abbildungen/projekte/vorträge/2026_hamburg_dielinke/AltGes_Hamburg_1.png"
