...
dichte_deutschland <- dichte_df %>%
  ...
  scale_x_continuous(breaks = c(250, 500, 750, 1000, 1250),
                     labels = ~paste0(.,"€")) +
  coord_cartesian(clip = "off") +
  labs(x = "Zimmermiete in deutschen Städten", y = NULL, title = NULL) +
  theme_dunkel() +
  theme(legend.position = "none",
        ...
        axis.text.y = element_markdown(family = axis_text_family,
                                       size = label_sizes, color = label_color))
...
file_save <- "/home/fabian/Schreibtisch/WG-Gesucht_Analyse/abbildungen/projekte/vorträge/2026_hamburg_dielinke/Ridgeline_Hamburg_kontext_2.png"
