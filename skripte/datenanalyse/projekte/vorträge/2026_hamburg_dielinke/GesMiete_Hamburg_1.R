...
plot_singleaxis <- ggplot() +
  ...
  with_outer_glow(
    geom_rect(data = daten_profile %>% filter(filter_label == T),
              ...
              fill = "#3f74a6", color = "gray15"),
    colour = "gray40", sigma = 8, expand = 1) +
  geom_rect(data = daten_profile %>% filter(filter_label == F),
            aes(xmin = -0.02, xmax = 0.02,
                ymin = anteil - 0.3, ymax = anteil + 0.3),
            fill = "gray25", color = NA) +
  annotate(geom = "text",
           x = 0, y = c(-1.2, 101.2), 
           label = c("0%", "100%"), 
...
file_save <- "/home/fabian/Schreibtisch/WG-Gesucht_Analyse/abbildungen/projekte/vorträge/2026_hamburg_dielinke/GesMiete_Hamburg_1.png"
