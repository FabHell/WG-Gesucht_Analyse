...
plot_alter_geschlecht <- alter_geschlecht %>%
  ...
  annotate("text",
           x = 41.5, y = 67,
           label = "Altersabhänige Unterschiede\nzwischen Geschlechtern",
           color = "#F2A65A", size = 4.25, hjust = 0, fontface = "italic",
           lineheight = 0.9, alpha = 0.6, family = "franklin") +
  annotate("curve",
           x = 41.5, y = 65.5, xend = 39.25, 
           yend = pull((alter_geschlecht[23,6] + alter_geschlecht[23,7]) / 2),
           curvature = -0.3, arrow = arrow(length = unit(0.2, "cm")),
           color = "#F2A65A", linewidth = 0.5, alpha = 0.6) +
  scale_y_continuous(limits = c(0,100),
                     labels = ~paste0(.x, "%")) +
...
Plot_gesamt_logo <- ggdraw(Plot_gesamt) +
  draw_image(
    "/home/fabian/Schreibtisch/WG-Gesucht_Analyse/daten/logos/DG_Logo_schwarzweiß.png",
    x = 0.955, y = -0.465,
    width = 0.03
  )


file_save <- "/home/fabian/Schreibtisch/WG-Gesucht_Analyse/abbildungen/projekte/vorträge/2026_hamburg_dielinke/AltGes_Hamburg_8.png"
