...
Histogram_Miete <- WGdaten_Stadt %>%
  ...
  geom_histogram(fill= "gray80", colour= "gray20", binwidth = 25) +
  scale_x_continuous(breaks = c(300,450,600,750),
                     limits = c(min(WGdaten_Stadt$gesamtmiete)-50,
                                max(WGdaten_Stadt$gesamtmiete)-25)) +
...
file_save <- "Abbildungen/Histogram_Miete_Rostock.png"
