...
Karte_erhobeneStädte <- ggplot() +
  ...
  geom_sf(data = Ländergrenzen %>% filter(shapeISO == "DEU"),
          fill = "transparent") +
  geom_sf(data = Städte, 
          size = 8, fill = "#FFA6A6", alpha = 0.8, shape = 21, color="gray20",
          linewidth = 5) +
  theme_void() +
...
file_save <- "C:\\Users\\hellm\\Desktop\\WG-Gesucht_Analyse\\abbildungen\\projekte\\broschueren\\präsentation_politik_hamburg\\Karte_erhobene_Städte.png"
ggsave(filename = file_save, plot = Karte_erhobeneStädte, 
       dpi = 300, width = 4.9, height = 6)
